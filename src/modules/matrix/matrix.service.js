...
class MatrixService {
  ...
  async createSubReason(input = {}) {
    ...
    return this.repository.withTransaction(async client => {
      ...
      if (
        clasificacion_pda_id !== undefined &&
        clasificacion_pda_id !== null &&
        clasificacion_pda_id !== ''
      ) {

        const parsedClassificationId =
          Number(
            clasificacion_pda_id
          );
        ...
        if (
          ...
          parsedClassificationId <= 0
        ) {
          throw MatrixService.writeError(
            'Clasificación PDA inválida',
            400
          );
        }
        ...
        if (!classification) {
          throw MatrixService.writeError(
            'La clasificación PDA indicada no existe o está inactiva',
            400
          );
        }
        ...
        /*
        * Compatibilidad temporal con la UI actual.
        * Mientras no envíe clasificacion_pda_id,
        * los nuevos submotivos usarán PROCESO.
        */
        classification =
          await this.repository
            .findPdaClassificationByCode(
              client,
              'PROCESO'
            );
        ...
        if (!classification) {
          throw MatrixService.writeError(
            'No existe la clasificación PDA PROCESO',
            500
          );
        }
      }
      ...
      const clasificacionLegacy =
        MatrixService
          .legacyClassificationFromCode(
            classification.codigo
          );

      return this.repository.insertSubReason(
        client,
        {
          attributeId:
            atributo_id,
  ...
  async updateSubReason(id, input = {}) {
    ...
    return this.repository.withTransaction(async client => {
      ...
      let classificationId =
        current.clasificacion_pda_id;


      if (
        input.clasificacion_pda_id !==
        undefined
      ) {
      ...
      if (classificationId) {
        ...
        /*
        * Compatibilidad excepcional con registros
        * anteriores a la migración 0029.
        */
        const legacy =
          String(
      ...
      return this.repository.updateSubReason(
        client,
        parsedId,
        {
          attributeId,
  ...
  static legacyClassificationFromCode(
    codigo
  ) {
    switch (
    String(codigo || '')
  ...
  async listPdaClassifications() {
    return this.repository.listPdaClassifications();
  }
