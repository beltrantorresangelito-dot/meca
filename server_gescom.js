...
// Función para encontrar Python portable automáticamente
function encontrarPython() {
    // Posibles ubicaciones de Python portable
    const posiblesUbicaciones = [
        // 🔴 PRIORIDAD 1: Tu Python portable específico
        'D:\\xampp\\htdocs\\gescot\\MECA\\python\\python.exe',
        'D:\\xampp\\htdocs\\gescot\\MECA\\python\\bin\\python.exe',
        'D:\\xampp\\htdocs\\gescot\\MECA\\PythonPortable\\python.exe',
        // 🔴 PRIORIDAD 2: Relativas al proyecto
        path.join(__dirname, '..', 'python', 'python.exe'),
        path.join(__dirname, '..', 'python', 'bin', 'python.exe'),
        path.join(__dirname, '..', 'PythonPortable', 'python.exe'),
        path.join(process.cwd(), 'python', 'python.exe'),
        path.join(process.cwd(), 'PythonPortable', 'python.exe'),
        // 🔴 PRIORIDAD 3: Ubicaciones comunes
        'C:\\PythonPortable\\python.exe',
        'C:\\PythonPortable\\bin\\python.exe',
        'C:\\Python\\python.exe',
        // 🔴 PRIORIDAD 4: Buscar en todo el sistema (lento, solo como último recurso)
        // Último recurso: usar 'python' del sistema
        'python',
        'python3'
    ];
    
    console.log('🔍 Buscando Python portable...');
    for (const ubicacion of posiblesUbicaciones) {
        // Si es 'python' o 'python3', no verificamos existencia
        if (ubicacion === 'python' || ubicacion === 'python3') {
            console.log(`   ℹ️ Usando comando del sistema: ${ubicacion}`);
            return ubicacion;
        }
        if (fs.existsSync(ubicacion)) {
            console.log(`   ✅ Python portable encontrado en: ${ubicacion}`);
            return ubicacion;
        }
    }
    
    // Fallback: usar 'python' del sistema
    console.warn('⚠️ No se encontró Python portable, usando "python" del sistema');
    console.warn('   💡 Si tienes Python portable, asegúrate de que esté en la carpeta correcta');
    return 'python';
}
...
const servidor = http.createServer(async (peticion, respuesta) => {
    ...
    // 🔴🔴🔴 PRIMERO: VERIFICAR RUTAS DE REPORTES (ANTES QUE NADA)
    // ======================================================
    // API - REPORTES - Ejecutar generación de reportes
    // ======================================================
    if (ruta === '/api/reportes/ejecutar' && metodo === 'POST') {
        console.log('[API] POST /api/reportes/ejecutar');
        ...
        peticion.on('end', async () => {
            try {
                ...
                const scriptPath = path.join(__dirname, '..', 'scripts_python', 'generar_reportes.py');
                
                // 🔴 ENCONTRAR PYTHON PORTABLE AUTOMÁTICAMENTE
                ...
                console.log(`📁 ¿Existe Python? ${fs.existsSync(pythonPath) || pythonPath === 'python' || pythonPath === 'python3'}`);
                console.log(`📁 ¿Existe Script? ${fs.existsSync(scriptPath)}`);
                console.log('='.repeat(60));
                ...
                if (!fs.existsSync(scriptPath)) {
                    ...
                    const alternativas = [
                        path.join(__dirname, 'generar_reportes.py'),
                        path.join(__dirname, '..', 'generar_reportes.py'),
                        path.join(process.cwd(), 'scripts_python', 'generar_reportes.py'),
                        path.join(process.cwd(), 'generar_reportes.py'),
                        'D:\\xampp\\htdocs\\gescot\\MECA\\scripts_python\\generar_reportes.py'
                    ];
                    ...
                    if (!encontrado) {
                        ...
                        respuesta.end(JSON.stringify({ 
                            ...
                            alternativas_buscadas: alternativas
                        }));
                        return;
                    }
                    
                    console.log(`✅ Script encontrado en: ${encontrado}`);
                    const finalScriptPath = encontrado;
                    
                    const { spawn } = require('child_process');
                    const pythonProcess = spawn(pythonPath, [finalScriptPath, '--tipo', tipo || 'todos'], {
                        cwd: path.dirname(finalScriptPath),
                        env: process.env
                    });
                    ...
                    pythonProcess.stdout.on('data', (data) => {
                        ...
                        stdout += texto;
                        console.log(`[Python STDOUT] ${texto}`);
                    });
                    
                    pythonProcess.stderr.on('data', (data) => {
                        const texto = data.toString();
                        stderr += texto;
                        console.error(`[Python STDERR] ${texto}`);
                    });
                    ...
                    pythonProcess.on('close', (code) => {
                        ...
                        respuesta.end(JSON.stringify({
                            success: code === 0,
                            message: code === 0 ? 'Reportes generados correctamente' : 'Error en la generación',
                            output: stdout,
                            error: stderr,
                            code: code,
                ...
                // 🔴 EJECUTAR CON LA RUTA CORRECTA
                const { spawn } = require('child_process');
                const pythonProcess = spawn(pythonPath, [scriptPath, '--tipo', tipo || 'todos'], {
        ...
        return;  // 🔴 CLAVE: Salir después de procesar la ruta
    }

    // ======================================================
    // API - REPORTES - Estado del sistema
    // ======================================================
    if (ruta === '/api/reportes/estado' && metodo === 'GET') {
        console.log('[API] GET /api/reportes/estado');
        ...
        try {
            const scriptPath = path.join(__dirname, '..', 'scripts_python', 'generar_reportes.py');
            const scriptExiste = fs.existsSync(scriptPath);
            
            const carpetaReportes = path.join(__dirname, '..', 'reportes_generados');
            const carpetaExiste = fs.existsSync(carpetaReportes);
            
            let ultimosReportes = [];
            if (carpetaExiste) {
                try {
                    const subcarpetas = ['pda_individual', 'mensuales_individuales', 'listas_capacitacion', 'listas_reiterativos'];
                    for (const sub of subcarpetas) {
                        const subPath = path.join(carpetaReportes, sub);
                        if (fs.existsSync(subPath)) {
                            const archivos = fs.readdirSync(subPath)
                                ...
                                .map(f => ({
                                    nombre: f,
                                    carpeta: sub,
                                    fecha: fs.statSync(path.join(subPath, f)).mtime
                                }));
                            ultimosReportes.push(...archivos);
                        }
                    }
                    ultimosReportes.sort((a, b) => b.fecha - a.fecha);
                    ultimosReportes = ultimosReportes.slice(0, 10);
                } catch (e) {
                    console.warn('Error leyendo reportes:', e);
                }
            }
            ...
            respuesta.end(JSON.stringify({
                ...
                script_python_existe: scriptExiste,
                carpeta_reportes_existe: carpetaExiste,
                ultimos_reportes: ultimosReportes.map(r => ({
                    nombre: r.nombre,
                    carpeta: r.carpeta,
                    fecha: r.fecha.toISOString()
                })),
                total_reportes: ultimosReportes.length
            }));
            
        } catch (error) {
            console.error('❌ Error en /api/reportes/estado:', error);
            respuesta.writeHead(500, { 'Content-Type': 'application/json' });
            respuesta.end(JSON.stringify({ 
                success: false, 
                error: error.message 
...
if (ruta.startsWith('/api/reglas-evaluacion/version/') && metodo === 'GET') {
    ...
    // ======================================================
    // API - MATRIZ VERSIONADA - OBTENER POR FECHA
    // ======================================================

    if (ruta === '/api/matriz/version-por-fecha' && metodo === 'GET') {
        console.log('[API] GET /api/matriz/version-por-fecha');
        ...
        console.log(`📡 Consultando matriz para fecha: ${fechaConsulta}`);
        
        try {
            // 1. Buscar la versión activa para la fecha
            const versionResult = await pool.query(`
                SELECT * FROM versiones_matriz 
                WHERE activa = true 
            ...
            if (versionResult.rows.length === 0) {
                // Fallback: buscar la versión activa más reciente
                ...
                if (fallbackResult.rows.length === 0) {
                    ...
                    respuesta.end(JSON.stringify({ 
                        success: false, 
                        error: 'No hay versiones de matriz activas' 
                    }));
                    return;
                }
                
                console.log(`   ⚠️ No hay versión para ${fechaConsulta}, usando la más reciente`);
                
                const version = fallbackResult.rows[0];
                
                // Obtener frentes de la versión
                const frentesResult = await pool.query(`
                ...
                // Extraer pesos
                let pesos = { ENC: 15, ECUF: 15, ECN: 70 };
                for (const frente of frentesResult.rows) {
                    const codigo = (frente.codigo || '').toUpperCase();
                    if (codigo === 'ENC' || codigo === 'CLIENTE') {
                        pesos.ENC = frente.peso_maximo || 15;
                    } else if (codigo === 'ECUF' || codigo === 'NEGOCIO') {
                        pesos.ECUF = frente.peso_maximo || 15;
                    } else if (codigo === 'ECN' || codigo === 'PROCESO') {
                        pesos.ECN = frente.peso_maximo || 70;
                    }
                }
            ...
            const version = versionResult.rows[0];
            console.log(`✅ Versión encontrada: ${version.version} (ID: ${version.id})`);
            
            // 2. Obtener los frentes de la versión
            const frentesResult = await pool.query(`
                SELECT id, codigo, nombre, peso_maximo 
                FROM version_frentes 
                WHERE version_id = $1 AND activo = true
            ...
            // 3. Extraer pesos de los frentes
            let pesos = { ENC: 15, ECUF: 15, ECN: 70 };
            let frentesMap = {};
            
            for (const frente of frentesResult.rows) {
                const codigo = (frente.codigo || '').toUpperCase();
                frentesMap[codigo] = frente.peso_maximo;
                console.log(`   📌 ${codigo}: ${frente.peso_maximo}%`);
            ...
            console.log(`   ✅ Pesos: ENC=${pesos.ENC}, ECUF=${pesos.ECUF}, ECN=${pesos.ECN}`);
            ...
            respuesta.end(JSON.stringify({
                ...
                descripcion: version.descripcion || '',
                frentes: frentesResult.rows,
                frentesMap: frentesMap,
                pesos: pesos,
                fecha_consulta: fechaConsulta
            }));
            
        } catch (error) {
            console.error('❌ Error en /api/matriz/version-por-fecha:', error);
            respuesta.writeHead(500, { 'Content-Type': 'application/json' });
            respuesta.end(JSON.stringify({ 
                success: false, 
                error: error.message 
    ...
    // ======================================================
    // API - MATRIZ VERSIONADA - OBTENER FRENTES DE UNA VERSIÓN
    // ======================================================

    if (ruta.match(/^\/api\/matriz\/versiones\/\d+\/frentes$/) && metodo === 'GET') {
        console.log('[API] GET /api/matriz/versiones/:id/frentes');
        ...
        // Extraer ID de la URL: /api/matriz/versiones/4/frentes
        const parts = ruta.split('/');
        // parts = ['', 'api', 'matriz', 'versiones', '4', 'frentes']
        const versionId = parseInt(parts[4]);
        ...
        console.log(`📡 Obteniendo frentes de versión ID: ${versionId}`);
        
        try {
            // Verificar que la versión existe
            const versionCheck = await pool.query(
                'SELECT id, version FROM versiones_matriz WHERE id = $1',
                [versionId]
            );
            ...
            console.log(`✅ Versión: ${versionCheck.rows[0].version}`);
            ...
            // Para cada frente, obtener sus atributos
            const frentes = [];
            for (const frente of frentesResult.rows) {
                const atributosResult = await pool.query(`
            ...
            respuesta.writeHead(200, { 'Content-Type': 'application/json' });
            respuesta.end(JSON.stringify(frentes));
            
        } catch (error) {
            console.error('❌ Error en /api/matriz/versiones/:id/frentes:', error);
            respuesta.writeHead(500, { 'Content-Type': 'application/json' });
            respuesta.end(JSON.stringify({ error: error.message }));
        }
        return;
